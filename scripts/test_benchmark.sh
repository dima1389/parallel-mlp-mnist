...
./build/train_omp --config "$cfg" --data "$DATA_DIR" --threads 2 --log "$LOG_DIR/omp_${cfg_name}_2t.log" > "$TMP" 2>&1
epoch_time=$(grep "Epoch" "$TMP" | tail -1 | grep -oP 'Time:\s+\K[0-9.]+')
echo "$cfg_name,openmp,2,$epoch_time" >> "$CSV"
echo "  OpenMP 2: $epoch_time s"

echo "Testing MPI (2 procs)..."
mpiexec -np 2 ./build/train_mpi --config "$cfg" --data "$DATA_DIR" --log "$LOG_DIR/mpi_${cfg_name}_2p.log" > "$TMP" 2>&1
epoch_time=$(grep "Epoch" "$TMP" | tail -1 | grep -oP 'Time:\s+\K[0-9.]+')
